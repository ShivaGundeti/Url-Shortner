...
export async function Login(data: AuthRequest) {
    const api = await axios.post("http://localhost:80/url/login", data,{withCredentials:true})
...
export async function Register(data: AuthRequest) {
    const api = await axios.post<RegisterResponse>("http://localhost:80/url/register", data,{withCredentials:true})
